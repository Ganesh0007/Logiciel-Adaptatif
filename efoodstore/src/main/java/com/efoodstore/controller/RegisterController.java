...
import com.efoodstore.model.BillingAddress;
import com.efoodstore.model.Customer;
import com.efoodstore.model.ShippingAddress;
    ...
    public String registerCustomerPost(@Valid @ModelAttribute("customer") Customer customer, BindingResult result,
        ...
        for (int i=0; i< customerList.size(); i++) {
            if(customer.getCustomerEmail().equals(customerList.get(i).getCustomerEmail())) {
                model.addAttribute("emailMsg", "Email existante");

                return "registerCustomer";
            }

            if(customer.getUsername().equals(customerList.get(i).getUsername())) {
                model.addAttribute("usernameMsg", "Le pseudo existe déjà");
